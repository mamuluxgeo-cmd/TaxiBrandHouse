...
async function login(u,p){const d=await call('login',{username:u,password:p});token=d.token;user=d.user||{name:u};localStorage.taxiToken=token;localStorage.taxiUser=JSON.stringify(user);await load();render()}
async function load(){
...
function driverList(arr){return arr.map(d=>`<div class="driver-card" onclick="select('${e(d.code)}')"><div><strong>${e(d.fullName)}</strong><small>${e(d.code)} • ${e(d.personalId)} • ${e(d.phone)}</small></div><b class="points ${cls(d.balance)}">${fmt(d.balance)}</b></div>`).join('')||`<div class="empty">მონაცემი არ არის</div>`}
...
function bind(){let q=document.getElementById('q');if(q)q.oninput=()=>{document.getElementById('rows').innerHTML=rows(filter(q.value))};let f=document.getElementById('find');if(f)f.oninput=()=>{document.getElementById('list').innerHTML=driverList(filter(f.value).slice(0,25))};let a=document.getElementById('amount');if(a){a.oninput=()=>{opAmount=a.value;render()};a.focus();a.selectionStart=a.selectionEnd=a.value.length}}
function select(code){selectedCode=code;opAmount='';render()}
async function saveOp(type){const selected=driverByCode(selectedCode);let amt=n(opAmount);if(!selected||amt<=0)return msg('შეავსე თანხა');try{await call(type=='sale'?'addSale':'addPayout',{driverCode:selected.code,amount:amt,points:amt});await load();render();msg('შენახულია')}catch(e){msg(e.message)}}
